...
export default class Weathrly extends Component {
  ...
  handleTenDaySubmit(place){
    $.get(  `http://api.wunderground.com/api/b261b543a03d4205/forecast10day/${place}.json`).then((weather)=>{
      ...
      this.setState({weather: this.state.tenDayWeather, location : this.state.location})
      this.storeLocal()
    })
    console.log(this.state.tenDayWeather.length)
  ...
  storeLocal(){
    localStorage.setItem('location', this.state.apiLocation, this.state.location)
  }
  ...
  render() {
    console.log("Location passed to WeathrlyRender ", this.state.location)
    return (
